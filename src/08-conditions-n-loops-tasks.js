...
function getCommonDirectoryPath(pathes) {
  const arrPath = pathes.map((el) => el.split('/'));
  const arrPath0 = arrPath[0];
  const res = [];
  for (let i = 0; i < arrPath0.length; i++) {
    if (arrPath.every((item) => item[i] === arrPath0[i])) {
      res.push(arrPath0[i]);
    }
  }
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < position.length; i++) {
    ...
    && (position[0][i] === 'X' || position[0][i] === '0')) {
      return position[0][i];
    }
  }

  const check = position[1][1];

  if (((check === position[0][0]) && (check === position[2][2]))
  // eslint-disable-next-line no-mixed-operators
  || ((check === position[2][0]) && (check === position[0][2]))
  // eslint-disable-next-line no-mixed-operators
  && (check === 'X' || check === '0')) {
    return check;
  }
